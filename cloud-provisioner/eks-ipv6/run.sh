...
OUT_DIR=$DIR/../../._output/env/eks-ipv6

UTILITY_DIR=$(realpath "$(dirname "${BASH_SOURCE[0]}")")
source $UTILITY_DIR/../../utility/common.sh

provision() {
    logger i:8 "Provisioning 3 IPv6-only EKS clusters" "#" "fg-yellow"
    ...
    EKS_CLUSTER_REGION=ap-southeast-2

    EAST_CLOUD_PROVIDER="eks-ipv6"
    WEST_CLOUD_PROVIDER="eks-ipv6"
    MGMT_CLOUD_PROVIDER="eks-ipv6"
    ...
    logger i:6 "Project=${PROJECT},Owner=${CLUSTER_OWNER},Region=eks:${EKS_CLUSTER_REGION}"
    ...
    echo "export EKS_CLUSTER_REGION=\"$EKS_CLUSTER_REGION\"" >>$OUT_DIR/env.sh
    ...
    terraform -chdir=$DIR/../terraform-aws-ipv6 init
    terraform -chdir=$DIR/../terraform-aws-ipv6 apply -var-file $OUT_DIR/tf-template/terraform.tfvars -state $DIR/terraform.tfstate -auto-approve

    sleep 2

    echo "export MGMT_CONTEXT=\"$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/terraform.tfstate -json | jq -r '.eks_ipv6_1_k8_kubeconfig_context.value')\"" >>$OUT_DIR/env.sh
    echo "export EAST_CONTEXT=\"$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/terraform.tfstate -json | jq -r '.eks_ipv6_2_k8_kubeconfig_context.value')\"" >>$OUT_DIR/env.sh
    echo "export WEST_CONTEXT=\"$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/terraform.tfstate -json | jq -r '.eks_ipv6_3_k8_kubeconfig_context.value')\"" >>$OUT_DIR/env.sh

    echo "export MGMT_CLUSTER=\"$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/terraform.tfstate -json | jq -r '.eks_ipv6_1_k8_cluster_name.value')\"" >>$OUT_DIR/env.sh
    echo "export EAST_CLUSTER=\"$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/terraform.tfstate -json | jq -r '.eks_ipv6_2_k8_cluster_name.value')\"" >>$OUT_DIR/env.sh
    echo "export WEST_CLUSTER=\"$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/terraform.tfstate -json | jq -r '.eks_ipv6_3_k8_cluster_name.value')\"" >>$OUT_DIR/env.sh

    KUBECONFIG_FILES=$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/terraform.tfstate -json | jq -r '.eks_ipv6_1_k8_kubeconfig.value'):$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/terraform.tfstate -json | jq -r '.eks_ipv6_2_k8_kubeconfig.value'):$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/terraform.tfstate -json | jq -r '.eks_ipv6_3_k8_kubeconfig.value')
    KUBECONFIG_ENV=""
...
clean() {
    logger i:8 "Destroying 3 IPv6-only EKS clusters" "#" "fg-yellow"

    if ! confirm "Are you sure you want to proceed with destroying the clusters ?"; then
        logger i "Ok, existing then ..."
        exit 0
    fi
    terraform -chdir=$DIR/../terraform-aws-ipv6 destroy -var-file $OUT_DIR/tf-template/terraform.tfvars -state $DIR/terraform.tfstate -auto-approve
}
