...
OUT_DIR=$DIR/../../._output/env/eks-aks-hybrid

UTILITY_DIR=$(realpath "$(dirname "${BASH_SOURCE[0]}")")
source $UTILITY_DIR/../../utility/common.sh

provision() {
    logger i:8 "Provisioning AKS & EKS clusters" "#" "fg-yellow"
    ...
    AKS_CLUSTER_REGION="Australia East"

    EAST_CLOUD_PROVIDER="aks"
    ...
    logger i:6 "Project=${PROJECT},Owner=${CLUSTER_OWNER},Region=eks:${EKS_CLUSTER_REGION}|aks:${AKS_CLUSTER_REGION}"
    ...
    echo "export AKS_CLUSTER_REGION=\"$AKS_CLUSTER_REGION\"" >>$OUT_DIR/env.sh
    ...
    echo "export EAST_CONTEXT=\"$(terraform -chdir=$DIR/../terraform-cloud-provisioner output -state $DIR/terraform.tfstate -json | jq -r '.aks_kubeconfig_context.value[0]')\"" >>$OUT_DIR/env.sh
    echo "export WEST_CONTEXT=\"$(terraform -chdir=$DIR/../terraform-cloud-provisioner output -state $DIR/terraform.tfstate -json | jq -r '.eks_kubeconfig_context.value[0]')\"" >>$OUT_DIR/env.sh
    echo "export MGMT_CONTEXT=\"$(terraform -chdir=$DIR/../terraform-cloud-provisioner output -state $DIR/terraform.tfstate -json | jq -r '.eks_kubeconfig_context.value[1]')\"" >>$OUT_DIR/env.sh

    echo "export EAST_CLUSTER=\"$(terraform -chdir=$DIR/../terraform-cloud-provisioner output -state $DIR/terraform.tfstate -json | jq -r '.aks_cluster_name.value[0]')\"" >>$OUT_DIR/env.sh
    echo "export WEST_CLUSTER=\"$(terraform -chdir=$DIR/../terraform-cloud-provisioner output -state $DIR/terraform.tfstate -json | jq -r '.eks_cluster_name.value[0]')\"" >>$OUT_DIR/env.sh
    echo "export MGMT_CLUSTER=\"$(terraform -chdir=$DIR/../terraform-cloud-provisioner output -state $DIR/terraform.tfstate -json | jq -r '.eks_cluster_name.value[1]')\"" >>$OUT_DIR/env.sh

    KUBECONFIG_FILES=$(terraform -chdir=$DIR/../terraform-cloud-provisioner output -state $DIR/terraform.tfstate -json | jq -r '.aks_kubeconfig.value'):$(terraform -chdir=$DIR/../terraform-cloud-provisioner output -state $DIR/terraform.tfstate -json | jq -r '.eks_kubeconfig.value')
    KUBECONFIG_ENV=""
...
clean() {
    logger i:8 "Destroying AKS & EKS clusters" "#" "fg-yellow"
