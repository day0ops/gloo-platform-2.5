...
OUT_DIR=$DIR/../../._output/env/eks-ipv4-ipv6-hybrid

UTILITY_DIR=$(realpath "$(dirname "${BASH_SOURCE[0]}")")
source $UTILITY_DIR/../../utility/common.sh

provision() {
    logger i:8 "Provisioning EKS clusters (2 IPv6 based & 1 IPv4 based)" "#" "fg-yellow"

    mkdir -p $OUT_DIR
    echo "#!/bin/sh" >$OUT_DIR/env.sh
    chmod +x $OUT_DIR/env.sh

    local default_eks_region="ap-southeast-2"
    ...
    envsubst <$DIR/ipv6-terraform.tfvars >$OUT_DIR/tf-template/ipv6-terraform.tfvars

    terraform -chdir=$DIR/../terraform-aws-ipv6 init
    terraform -chdir=$DIR/../terraform-aws-ipv6 apply -var-file $OUT_DIR/tf-template/ipv6-terraform.tfvars -state $DIR/ipv6-terraform.tfstate -auto-approve
    ...
    echo "export MGMT_CONTEXT=\"$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/ipv6-terraform.tfstate -json | jq -r '.eks_ipv6_1_k8_kubeconfig_context.value[0]')\"" >>$OUT_DIR/env.sh
    echo "export WEST_CONTEXT=\"$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/ipv6-terraform.tfstate -json | jq -r '.eks_ipv6_2_k8_kubeconfig_context.value[0]')\"" >>$OUT_DIR/env.sh
    ...
    echo "export MGMT_CLUSTER=\"$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/ipv6-terraform.tfstate -json | jq -r '.eks_ipv6_1_k8_cluster_name.value[0]')\"" >>$OUT_DIR/env.sh
    echo "export WEST_CLUSTER=\"$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/ipv6-terraform.tfstate -json | jq -r '.eks_ipv6_2_k8_cluster_name.value[0]')\"" >>$OUT_DIR/env.sh
    ...
    KUBECONFIG_FILES=$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/ipv6-terraform.tfstate -json | jq -r '.eks_ipv6_1_k8_kubeconfig.value[0]'):$(terraform -chdir=$DIR/../terraform-aws-ipv6 output -state $DIR/ipv6-terraform.tfstate -json | jq -r '.eks_ipv6_2_k8_kubeconfig.value[0]'):$(terraform -chdir=$DIR/../terraform-cloud-provisioner output -state $DIR/terraform.tfstate -json | jq -r '.eks_kubeconfig.value')
...
clean() {
    logger i:8 "Ready to clean the EKS clusters" "#" "fg-yellow"
    ...
    terraform -chdir=$DIR/../terraform-aws-ipv6 destroy -var-file $OUT_DIR/tf-template/ipv6-terraform.tfvars -state $DIR/ipv6-terraform.tfstate -auto-approve
